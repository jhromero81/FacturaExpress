  ...
  <div class="row" style="margin-bottom:0;">
    ...
    <div class="col s12 l4" style="padding-bottom:18px;">
      <div class="payment-panel">
        ...
        <div class="pay-row discount-row"><span class="pay-lbl">Descuento</span>
          <div class="discount-box">
            <div class="discount-presets">
              <button type="button" class="discount-chip" data-pct="0" onclick="setDescuento(0)">0%</button>
              <button type="button" class="discount-chip" data-pct="5" onclick="setDescuento(5)">5%</button>
              <button type="button" class="discount-chip" data-pct="10" onclick="setDescuento(10)">10%</button>
              <button type="button" class="discount-chip" data-pct="15" onclick="setDescuento(15)">15%</button>
              <button type="button" class="discount-chip" data-pct="20" onclick="setDescuento(20)">20%</button>
            </div>
            <div class="discount-slider-row">
              <input type="range" id="descuentoSlider" min="0" max="100" value="0" oninput="setDescuento(this.value)">
              <span class="discount-pct" id="descuentoPct">0%</span>
            </div>
            <div class="discount-amount-row">
              <span class="discount-monto" id="descuentoMonto">-$0</span>
              <span class="discount-hint">desliza o elige un valor</span>
            </div>
          </div>
        </div>
...
function setDescuento(pct) {
  pct = Math.min(100, Math.max(0, parseInt(pct) || 0));
  document.getElementById('descuentoSlider').value = pct;
  document.getElementById('descuentoPct').textContent = pct + '%';
  var chips = document.querySelectorAll('.discount-chip');
  chips.forEach(function(ch) {
    ch.classList.toggle('active', parseInt(ch.getAttribute('data-pct')) === pct);
  });
  actualizarPago();
}
...
function actualizarPago() {
  ...
  var slider = document.getElementById('descuentoSlider');
  var pct = Math.min(100, Math.max(0, parseInt(slider.value) || 0));
  slider.value = pct;
  document.getElementById('descuentoPct').textContent = pct + '%';
  var chips = document.querySelectorAll('.discount-chip');
  chips.forEach(function(ch) {
...
function validarCarrito() {
  ...
  dtoInput.value = document.getElementById('descuentoSlider').value;
