...
public class FacturaServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        switch (action) {
            ...
            case "detalle":
                ...
                break;
            case "descargarPdf":
                descargarPdf(req, resp);
                break;
            case "descargarXml":
                descargarXml(req, resp);
                break;
            case "eliminar":
    ...
    private void guardarFactura(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            factura.setUsuario(AuthUtil.getUsuario(req));
    ...
    // Descarga el PDF almacenado de una factura
    private void descargarPdf(HttpServletRequest req, HttpServletResponse resp)
            throws IOException {
        try {
            Long id = Long.parseLong(req.getParameter("id"));
            Optional<Factura> facturaOpt = facturaService.buscarPorId(id);
            if (facturaOpt.isEmpty() || facturaOpt.get().getPdf() == null) {
                resp.sendError(HttpServletResponse.SC_NOT_FOUND);
                return;
            }
            byte[] pdf = facturaOpt.get().getPdf();
            resp.setContentType("application/pdf");
            resp.setHeader("Content-Disposition", "attachment; filename=\"factura_" + id + ".pdf\"");
    ...
    // Descarga el XML almacenado de una factura
    private void descargarXml(HttpServletRequest req, HttpServletResponse resp)
            throws IOException {
        try {
            Long id = Long.parseLong(req.getParameter("id"));
            Optional<Factura> facturaOpt = facturaService.buscarPorId(id);
            if (facturaOpt.isEmpty() || facturaOpt.get().getXml() == null) {
                resp.sendError(HttpServletResponse.SC_NOT_FOUND);
                return;
            }
            byte[] xml = facturaOpt.get().getXml().getBytes(StandardCharsets.UTF_8);
            resp.setContentType("application/xml");
            resp.setHeader("Content-Disposition", "attachment; filename=\"factura_" + id + ".xml\"");
